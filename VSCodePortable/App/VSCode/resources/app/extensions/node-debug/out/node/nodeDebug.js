...
var NodeDebugSession = (function (_super) {
    ...
    NodeDebugSession.prototype.launchRequest = function (response, args) {
        ...
        if (runtimeExecutable) {
            ...
            if (!terminal_1.Terminal.isOnPath(NodeDebugSession.NODE)) {
                this.sendErrorResponse(response, 2001, localize(11, null, '{_runtime}'), { _runtime: NodeDebugSession.NODE });
                return;
            }
        ...
        if (NodeDebugSession.isJavaScript(programPath)) {
            ...
            if (!this._sourceMaps) {
                this.sendErrorResponse(response, 2002, localize(13, null, '{path}'), { path: programPath });
                return;
            }
            this._sourceMaps.MapPathFromSource(programPath).then(function (generatedPath) {
                if (!generatedPath) {
                    _this.sendErrorResponse(response, 2003, localize(14, null, '{path}', 'outDir'), { path: programPath });
                    return;
                }
                _this.log('sm', "launchRequest: program '" + programPath + "' seems to be the source; launch the generated file '" + generatedPath + "' instead");
                programPath = generatedPath;
            ...
            if (this._console === 'integratedTerminal' && this._supportsRunInTerminalRequest) {
                ...
                this.runInTerminalRequest(termArgs, NodeDebugSession.RUNINTERMINAL_TIMEOUT, function (runResponse) {
                    if (runResponse.success) {
                        ...
                        _this.sendErrorResponse(response, 2011, localize(16, null, '{_error}'), { _error: runResponse.message });
                ...
                terminal_1.Terminal.launchInTerminal(workingDirectory, launchArgs, args.env).then(function (term) {
                    ...
                    _this.sendErrorResponseWithInfoLink(response, 2011, localize(17, null, '{_error}'), { _error: error.message }, error.linkId);
    ...
    NodeDebugSession.prototype.completionsRequest = function (response, args) {
        ...
        var postfix = line.substring(column);
        var expression;
    ...
    NodeDebugSession.prototype.sendAttributeMissingErrorResponse = function (response, attribute) {
        this.sendErrorResponse(response, 2005, localize(42, null, attribute));
    };
    /**
    ...
    NodeDebugSession.RUNINTERMINAL_TIMEOUT = 3000;
