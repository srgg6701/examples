...
var SourceMaps = (function () {
    ...
    SourceMaps.prototype._rawSourceMap = function (uri, pathToGenerated) {
        ...
        if (u.protocol === 'http:' || u.protocol === 'https:') {
            ...
            var hash = CRYPTO.createHash('sha256').update(uri).digest('hex');
            var promise = this._httpCache.get(hash);
            if (!promise) {
                var cache_path = Path.join(OS.tmpdir(), 'com.microsoft.VSCode', 'node-debug', 'sm-cache');
                var path_1 = Path.join(cache_path, hash);
                promise = Promise.resolve(FS.existsSync(path_1)).then(function (exists) {
                    if (exists) {
                        return _this._readFile(path_1);
                    }
                    var options = {
                        url: uri,
                        followRedirects: 5
                    ...
                    return XHR.xhr(options).then(function (response) {
                        return _this._writeFile(path_1, response.responseText);
